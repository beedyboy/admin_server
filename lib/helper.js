...
generateToken: (user) => {
    ...
    try {
      const id = user.id; 
    var cipherText = crypto.AES.encrypt(id.toString(), util.SECRET).toString(); 
    return cipherText;
    } catch(err) {
      console.log('error', err);
    }
...
setSignature: (token, col, id) => {
  let reply = false;
   knex('signatures').where(col, id).update( 'token', token).then( data => {
              if (data){
                reply = true; }
                 else {reply = false; }
               });
...
actDCTLogin: async ( col, id, value) => {
  ...
  var reply = false;
   ...
   await knex('logins').where(col, id).update('status', value)
